...
+ (nullable NSNumber *)computeBetweenVector1:(NSArray<NSNumber *> *)u andVector2:(NSArray<NSNumber *> *)v isFloat:(BOOL)isFloat error:(NSError **)error {
    if (u.count != v.count) {
        [MPPCommonUtils
...
+ (nullable NSNumber *)computeBetweenEmbedding1:(MPPEmbedding *)embedding1 andEmbedding2:(MPPEmbedding *)embedding2 error:(NSError **)error {
  ...
  if (embedding1.floatEmbedding && embedding2.floatEmbedding) {
    return [MPPCosineSimilarity computeBetweenVector1:embedding1.floatEmbedding andVector2:embedding2.floatEmbedding isFloat:YES error:error];
  }

  if (embedding1.quantizedEmbedding && embedding2.quantizedEmbedding) {
    return [MPPCosineSimilarity computeBetweenVector1:embedding1.quantizedEmbedding andVector2:embedding2.quantizedEmbedding isFloat:NO error:error];
  }

  [MPPCommonUtils
        createCustomError:error
                 withCode:MPPTasksErrorCodeInvalidArgumentError
              description:@"Cannot compute cosine similarity between quantized and float embeddings."];
  return nil;
