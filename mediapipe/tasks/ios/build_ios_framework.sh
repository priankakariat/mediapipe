...
EOF
 local FRAMEWORK_PATH=$(bazel cquery -c opt --config=ios_fat --define=MEDIAPIPE_DISABLE_GPU=1 --define=IOS_STATIC_FRAMEWORK=1 $1 \
...
function build_ios_api_framework {
  ...
  "${BAZEL}" build -c opt --config=ios_fat --define=MEDIAPIPE_DISABLE_GPU=1 --define=IOS_STATIC_FRAMEWORK=1 ${FULL_TARGET}
