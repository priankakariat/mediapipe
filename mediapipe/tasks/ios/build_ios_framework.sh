...
# local OUTPUT_PATH=$(bazel cquery -c opt --define=MEDIAPIPE_AVOID_LINKING_GRAPHS=1 --define=MEDIAPIPE_DISABLE_GPU=1  $1 \
...
function build_target {
  # Build using the cquery command passed as argument.
...
function create_framework_archive {
  ...
  local SIMULATOR_GRAPH_LIBRARY_PATH="${FRAMEWORKS_DIR}/${GRAPH_LIBRARIES_DIR}/${FRAMEWORK_NAME}_simulator_graph_lib.a"
  ...
  local IOS_DEVICE_GRAPH_LIBRARY_PATH="${FRAMEWORKS_DIR}/${GRAPH_LIBRARIES_DIR}/${FRAMEWORK_NAME}_device_graph_lib.a"
...
cd "${MPP_ROOT_DIR}"
build_ios_frameworks_and_libraries 1
