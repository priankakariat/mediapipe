...
/**
 * Get the category label list of the `InteractiveSegmenter` can recognize. For CATEGORY_MASK type, the
 ...
 *
 * @return A new instance of `InteractiveSegmenter` with the given options. `nil` if there is an error
...
regionOfInterest:(MPPRegionOfInterest *)regionOfInterest
                                             error:(NSError **)error NS_SWIFT_NAME(segment(image:regionOfInterest:));
