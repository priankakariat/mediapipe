...
static NSString *const kTaskGraphName =
    @"mediapipe.tasks.vision.image_interactive.InteractiveSegmenterGraph";
...
- (nullable MPPImageSegmenterResult *)segmentImage:(MPPImage *)image regionOfInterest:(MPPRegionOfInterest *)regionOfInterest error:(NSError **)error {
  return [self segmentImage:image regionOfInterest:regionOfInterest shouldCopyOutputMaskPacketData:YES error:error];
}
...
regionOfInterest:(MPPRegionOfInterest *)regionOfInterest
  ...
  NSError *error = nil;                                    
  MPPImageSegmenterResult *result = [self segmentImage:image regionOfInterest:regionOfInterest shouldCopyOutputMaskPacketData:NO error:&error];
...
- (nullable MPPImageSegmenterResult *)
    segmentImage:(MPPImage *)image regionOfInterest:(MPPRegionOfInterest *)regionOfInterest
                           shouldCopyOutputMaskPacketData:(BOOL)shouldCopyMaskPacketData error:(NSError **)error {
...
std::optional<PacketMap> inputPacketMap = [_visionTaskRunner inputPacketMapWithMPPImage:image
   ...
   std::optional<Packet> renderDataPacket = [MPPVisionPacketCreator createRenderDataPacketWithRegionOfInterest:regionOfInterest error:error];
  ...
  // inputPacketMap.value().insert(std::pair<std::string, Packet>(kRoiInStreamName.cppString, renderDataPacket.value()));
           
  std::optional<PacketMap> outputPacketMap = [_visionTaskRunner processPacketMap:inputPacketMap.value() error:error];

  return [MPPInteractiveSegmenter imageSegmenterResultWithOptionalOutputPacketMap:outputPacketMap
  ...
  return [MPPImageSegmenterResult
      imageSegmenterResultWithConfidenceMasksPacket:outputPacketMapValue[kConfidenceMasksStreamName
                                ...
                                qualityScoresPacket:outputPacketMapValue[kQualityScoresStreamName
                                                                        .cppString]
                            timestampInMilliseconds:outputPacketMapValue[kImageOutStreamName.cppString]
                                                        .Timestamp()
                                                        .Value() /
