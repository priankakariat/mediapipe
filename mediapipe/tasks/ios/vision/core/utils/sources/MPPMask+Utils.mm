...
@implementation MPPMask (Utils)

+ (MPPMask *)maskFromMediapipeImage:(mediapipe::Packet)mediapipePacket {

  const isImage = mediapipePacket.ValidateAsType<mediapipe::Image>().ok();
  const ImageFrame& imageFrame = isImage ? mediapipePacket.Get<Image>().GetImageFrameSharedPtr().get() :  mediapipePacket.Get<ImageFrame>();
  if (!imageFrame.IsContiguous()) {
    return nil;
  }
